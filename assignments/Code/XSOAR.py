...
class Client():
    ...
    def fetch_all_issue(self):


        headers = {
           "Accept": "application/json"
        }
        data = requests.get(url = "https://kunal1122.atlassian.net/rest/api/2/project",auth = ("khadke1122@gmail.com", "rp1cn7zsnVoLTKsKaB7w3639"), headers=headers)
        data = data.json()
        url = "https://kunal1122.atlassian.net/rest/api/2/search"
        ...
        for p1 in data:
            ...
            res = requests.get(url=url, headers=headers,params=query, auth=("khadke1122@gmail.com", "rp1cn7zsnVoLTKsKaB7w3639"))
            res = res.json()

            issues = res["issues"]

            for i in issues:
                Iss = {"issue_Id": i["id"], "issue_key": i["key"], "issue_status": i["fields"]["status"]["name"],  "project_name": p1["name"]}
...
def jira_issues(client: Client) -> CommandResults:

    JiraIssue = client.fetch_all_issue()


    return CommandResults(outputs_prefix='all_issues_fromjira',outputs = JiraIssue, readable_output = tableToMarkdown(name = "issues",
    t = JiraIssue))
...
def main() -> None:
    ...
    try:
        ...
        elif command == 'fetch-all-issues':
            return_results(jira_issues(client))
        else:
